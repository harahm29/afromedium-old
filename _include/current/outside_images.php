...
class OutsideImages
{
    ...
    static public function filter_to_html($text, $start_tag, $end_tag, $a_class, $target = '_blank', $isCalcMaxWidth = false, $prfId = '', $imMsg = false)
    {
        global $g;
        global $sitePart;
        ...
        foreach ($ids as $id){
        	...
        	if ($image){
                ...
                if ($isEdgeWall && !$imMsg) {
                    ...
                    $tagHtml = $start_tag . '<a target="' . $target . '" class="' . $a_class . '" href="' . $image_b . '"><img ' . self::$addScriptLoad . $addStyleLoad . ' data-src-b="' . $image_b . '" id="' . $tagId . '" src="' . $image_th . '" alt=""/></a>' . $end_tag;
                }
